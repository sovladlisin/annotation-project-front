...
import { GET_CORPUS_PLACES, GET_CORPUS_OBJECTS, GET_CORPUS_CLASSES, GET_CORPUS_AUTHORS, corpusDispatchTypes, CREATE_CORPUS, DELETE_CORPUS, GET_CORPUSES, TCorpus, UPDATE_CORPUS, GET_CORPUS_RESOURCES, ADD_CORPUS_AUTHOR, DELETE_CORPUS_AUTHOR, ADD_CORPUS_PLACE, DELETE_CORPUS_PLACE } from "../../actions/models/corpuses/types"
...
const corpusReducer = (state: IDefaultState = defaultState, action: corpusDispatchTypes): IDefaultState => {
    switch (action.type) {
        ...
        case ADD_CORPUS_AUTHOR:
            return {
                ...state,
                corpus_authors: [...state.corpus_authors, action.payload]
            }
        case DELETE_CORPUS_AUTHOR:
            return {
                ...state,
                corpus_authors: state.corpus_authors.filter(a => a.id != action.payload)
            }
        case ADD_CORPUS_PLACE:
            return {
                ...state,
                corpus_places: [...state.corpus_places, action.payload]
            }
        case DELETE_CORPUS_PLACE:
            return {
                ...state,
                corpus_places: state.corpus_places.filter(p => p.id != action.payload)
            }
        default:
            return state;
    }
